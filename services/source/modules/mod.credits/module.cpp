...
namespace {
   ...
   class Module : public Exordium::Module {
    private:
      ...
      Exordium::CreditsModule::Service* service;
	...
	{};
	 ...
	 (void)Kine::langs().
	   deregisterMap(Exordium::CreditsModule::Language::tagMap);
	 ...
	 // Try to register the language map before we need it..
	 (void)Kine::langs().
	   registerMap(Exordium::CreditsModule::Language::tagMap);

	 // Make a new service
	 if ((service = new Exordium::CreditsModule::Service(configData, s)) ==
	     0) {
	    return false;
	 }
