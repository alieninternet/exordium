...
namespace {
   ...
   class Module : public Exordium::Module {
    private:
      ...
      Exordium::LoveModule::Service* service;
	...
	{};
	 ...
	 // Make a new service
	 if ((service = new Exordium::LoveModule::Service(configData, s))
	     == 0) {
	    return false;
	 }
	 ...
	 unsigned int i = 0;
	 while (Exordium::LoveModule::Tables::tables[i] != 0) {
	    // Try to affirm this table..
	    if (!s.getDatabase().
		affirmTable(*(Exordium::LoveModule::Tables::tables[i]))) {
	       s.logLine(String("Unable to affirm mod_love database table '") +
			 Exordium::LoveModule::Tables::tables[i]->name + "'",
