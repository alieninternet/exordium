...
namespace {
   ...
   class Module : public Exordium::Module {
    private:
      ...
      Exordium::NickModule::Service* service;
	...
	{};
	 ...
	 (void)Kine::langs().
	   deregisterMap(Exordium::NickModule::Language::tagMap);
	 ...
	 (void)Kine::langs().
	   registerMap(Exordium::NickModule::Language::tagMap);
	 
	 // Attempt to affirm our database tables..
	 unsigned int i = 0;
	 while (Exordium::NickModule::Tables::tables[i] != 0) {
	    // Try to affirm this table..
	    if (!s.getDatabase().
		affirmTable(*(Exordium::NickModule::Tables::tables[i]))) {
	       s.logLine(String("Unable to affirm mod_nick database "
				"table '") +
			 Exordium::NickModule::Tables::tables[i]->name + "'",
	 ...
	 // Make a new service
	 if ((service = new Exordium::NickModule::Service(configData, s)) ==
	     0) {
	    return false;
	 }
